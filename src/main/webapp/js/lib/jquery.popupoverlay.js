...
;(function ($) {
    ...
    var methods = {
        ...
        _initonce: function (el) {
            ...
            if ((options.background) && (!$('#' + el.id + '_background').length)) {
                ...
                if (options.setzindex && !options.autozindex) {
                    $background.css('z-index', '10000000');
                }
            ...
            $(document).on('click', closeelement, function (e) {
                methods.hide(el);
                //added by pzare to hide niceScroll on popup close for country Master
                //$(".popup .content").niceScroll().hide();
                e.preventDefault();
            });
    ...
    /*$(document).on('click', function (event) {
        if(stack.length) {
            var elementId = stack[stack.length - 1];
			var datebox = "dp-popup"
            var el = document.getElementById(elementId);

            if ($(el).data('popupoptions').blur && !$(event.target).parents().andSelf().is('#' + elementId) && $(el).data('popup-visible') && event.which !== 2 && !$(event.target).parents().andSelf().is('#' + datebox) ) {
                methods.hide(el);
    ...
    $.fn.popup = function (customoptions) {
        return this.each(function () {
        	//---SonarQube Issue 31 July 2017 1 var $el 
            var $el = $(this);
